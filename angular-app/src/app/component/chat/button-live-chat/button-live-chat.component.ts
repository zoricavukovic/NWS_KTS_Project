...
export class ButtonLiveChatComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.isLoggedIn()) {
      this.chatRoomSubscription = this.chatRoomService.getUserChatRoom(this.authService.getCurrentUser.email).subscribe(
        res => {
          this.chatRoom = res;
  ...
  updateNotifications(): void {
    this.numOfNotifications = this.chatRoomService.getNumOfNotSeenMessages(this.chatRoom, this.isAdmin);
  }
