...
public interface DrivingRepository extends JpaRepository<Driving, Long> {
    ...
    @Query(value = "select d from Driving d left join fetch d.route r left join fetch r.destinations dest left join fetch d.usersPaid up left join fetch d.users u where u.id = ?1")
