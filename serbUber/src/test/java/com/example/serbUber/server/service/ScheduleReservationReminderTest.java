...
public class ScheduleReservationReminderTest {
    ...
    public void shouldSendReminderWhenDrivingIsStartingSoonAndThereIsNoPreviousReminder() {
        ...
        doNothing().when(webSocketService).sendReservationReminder(new HashSet<>(), 15);
        ...
        verify(webSocketService, times(2)).sendReservationReminder(new HashSet<>(), 15);
    ...
    public void shouldSendReminderWhenDrivingIsStartingSoonAndThereIsPreviousReminder() {
        ...
        verify(webSocketService, times(1)).sendReservationReminder(new HashSet<>(), 15);
        verify(drivingService, times(1)).save(DRIVING_SHOULD_RECEIVE_FIRST_NOTIFICATION_3);
    }

    @Test
    @DisplayName("T2-Should not send reminder, time difference is bigger than 15 minutes")
    public void shouldNotSendReminderTooBigTimeDifferece() {

        when(drivingService.getAllReservations()).thenReturn(
                List.of(DRIVING_SHOULD_NOT_RECEIVE_REMINDER)
        );

        scheduleReservationReminder.remindPassengerForReservation();

        verify(webSocketService, times(0)).sendReservationReminder(new HashSet<>(), 15);
        verify(drivingService, times(0)).save(DRIVING_SHOULD_NOT_RECEIVE_REMINDER);
    }
